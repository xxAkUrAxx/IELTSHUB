      ...
      return [
        {
          ...
          title: group.title || "Matching Information (to paragraphs)",
          questionRange: buildTfngQuestionRange(group.items || []),
          instructions: `Look at the following statements (Questions ${buildTfngQuestionRange(
            group.items || []
          ...
          questions: (group.items || []).map((item) => ({
            ...
            correctParagraph: item.correctParagraph,
          })),
        },
      ];
    }
    ...
    const sectionAnswers = section.questions.flatMap((questionGroup) => {
      if (questionGroup.type !== "TFNG") {
        ...
        return questionGroup.questions.map((question) => [
          ...
          {
            type: "MATCHING_INFORMATION",
            correctAnswer: question.correctParagraph,
